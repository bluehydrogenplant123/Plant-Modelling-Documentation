...
> The accepted workbook/runtime pointer for the current merged version lives in
...
Last updated at version: `6.1.0`
Source of truth: `src/src/backend/utils/schemaMigrations.ts` -> `CURRENT_SCHEMA_VERSION`

Note: the active compatibility axis is now `v6.1.0`. This is a no-op schema
boundary on top of the clean `v6.0.0` reset from the
`feature/stable-version6.1` code branch: the persisted shape below is unchanged,
but saved diagrams stamped `6.0.0` are considered upgrade candidates for the
current apr-6-2026 runtime baseline. Older experimental `6.2/6.3` schema labels
are not part of the live executable history.
...
payload {
  name: string
  canvas: {
    schemaVersion: string         // "6.1.0"
    runtimeBaselineRef?: string   // current imported Excel/runtime baseline
    runtimeBaselineImportedAt?: string
    nodes: [...]
    edges: [...]
...
Diagram {
  ...
  type: Int (default 0), duration?: Float (default 1),
...
TpNodeVers {
  ...
  fromTp: Int, toTp: Int,
  duration?: Float, durationUnit?: String,
  modelVersion: String
}
