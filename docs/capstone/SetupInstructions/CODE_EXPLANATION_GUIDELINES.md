...
The current active user-facing documentation surface is:
...
- Treat `docs/SetupInstructions/` as the active current docs surface.
...
- Keep this standards page in current docs so contributors can find the writing rules before creating new explanations.
...
Current pages in `docs/SetupInstructions/` use frontmatter. New Docusaurus-rendered current docs should use frontmatter where the target docs surface expects it:
