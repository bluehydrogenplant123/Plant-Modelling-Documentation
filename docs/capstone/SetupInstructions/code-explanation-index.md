...
Use this page as the current entry point for HyProNet feature and workflow explanations. Every linked page below lives in the active documentation surface:
...
- [HyProNet Installation Guide](./HYPRONET_INSTALLATION_GUIDE.md): current detailed setup, environment, workbook import, and real solver connectivity guide.
- [HyProNet Streamlined Guide](./STARTUP_GUIDE.md): concise current startup, shutdown, update, and real solver notes.

## Navigation Rule

For current feature documentation, start in `docs/SetupInstructions/` and follow only current links from this index or from related current pages. Use `docs-archive/PreviousDoc/CodeExplanation/` only for historical comparison, and do not link new current docs to misplaced `version6.1/` CodeExplanation copies.
