...
5. Review or update only the fields that are enabled in the selected panel.
6. Close the panel when finished.

## Result

The selected Multi-TP economic panel opens for the current diagram. If cost controls are disabled or no Multi-TP ranges exist, the panel is read-only or unavailable.
