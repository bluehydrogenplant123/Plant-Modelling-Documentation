...
## Async Thunks
- `fetchModelVersion(nodeId)` 鈥?GET `/api/data/nodes/{nodeId}`; stores returned `modelVersion`.
- `saveAllChanges(diagramId)` 鈥?saves all dirty nodes; PUT when nodeId looks canonical (24-hex), otherwise POST to create and possibly remap ids; returns `{ savedCount, replacements }`.
- `saveNode(nodeId)` 鈥?PUT single `modelVersion` for nodeId.
...
- `renameNodeCacheEntry({ oldNodeId, newNodeId, modelVersion })` 鈥?remaps cache/dirties/loading entries when backend creates new ids.
