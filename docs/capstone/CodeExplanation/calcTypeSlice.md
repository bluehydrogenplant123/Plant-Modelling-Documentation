...
## State
- `type: CalcType` where `CalcType` 鈭?`['Simulation', 'Optimization', 'DataRec', 'ParamUpdt']` (default `'Simulation'`).

## Actions
- `updateCalcType(type)` 鈥?sets state when payload is in `CALC_TYPES`; logs a warning otherwise.
