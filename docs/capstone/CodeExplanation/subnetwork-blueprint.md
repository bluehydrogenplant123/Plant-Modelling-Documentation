...
### 1.5 Subnetwork Editor Auto鈥慡ave
...
- After the auto鈥憇ave completes (or if already saved), the Subnetwork Editor modal opens.
...
buildSubnetworkPortMap()
    鈫?(builds internal resolution map)
Process Canvas Nodes
    鈫?
isSubnetworkInstanceNode() 鈫?Skip wrapper (mark in skippedSubnetworkNodes)
    鈫?
Process Canvas Edges
    鈫?
resolveEndpoint() 鈫?Uses subnetworkPortMap to find internal nodes
    鈫?
...
Collect instance diagram IDs from base canvas
    鈫?(subnetwork nodes reference instance diagrams)
...
For each instance diagram:
    鈹溾攢 updateCanvasAndParametersFromOutput() [instance canvas]
    鈹?  鈫?
    鈹?  Update instance nodes
    鈹溾攢 backpropagateFromInstanceToParent()
    鈹?  鈫?
    鈹?  Copy values from internal 鈫?wrapper node in parent
    鈫?
...
- Includes ReactFlow ID 鈫?Document ID 鈫?ModelVersion mappings
...
**During computation:**
1. `buildSubnetworkPortMap` resolves all external鈫抜nternal port mappings
