...
The current `apr-6-2026.xlsx` workbook also contains sheets such as `EQ type` and `SYS Node Variable Spec`. `excel_to_csv.py` will export unmapped sheets using their original names, but `migrate.py` ignores them unless a current `process_*` function reads that CSV.
...
2. From `src/`, the contributor runs `bash run-all.sh apr-6-2026.xlsx`.
...
The setup guides also allow `./run-all.sh apr-6-2026.xlsx` when the script is executable.
...
```powershell
git diff --check -- docs/SetupInstructions/CodeExplanation/excel-import-pipeline.md
```
...
- `docs/SetupInstructions/Installation/STARTUP_GUIDE.md`
- `docs/SetupInstructions/Installation/HYPRONET_INSTALLATION_GUIDE.md`
- `docs/SetupInstructions/CodeExplanation/backend-data-routes-and-persistence.md`
- `docs/SetupInstructions/CodeExplanation/compute-solver-callback-and-results.md`
