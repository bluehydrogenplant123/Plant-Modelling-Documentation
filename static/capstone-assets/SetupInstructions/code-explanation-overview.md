...
This page explains what the current HyProNet `CodeExplanation` set covers and how contributors should read it. The active documentation surface is:
...
Current pages live in `docs/SetupInstructions/`. Contributors should start with the current pages in this folder and follow current links only.
...
| Import and setup boundary | [Excel Import Pipeline Code Explanation](./excel-import-pipeline.md), [HyProNet Installation Guide](./HYPRONET_INSTALLATION_GUIDE.md), [HyProNet Streamlined Guide](./STARTUP_GUIDE.md) | Workbook-to-CSV normalization, Prisma setup, Docker services, PostgreSQL catalog writes, and local runtime setup. |
