...
```bash
docker compose run --rm python-runner /app/run_python_import.sh "/app/excel-sheets/apr-6-2026.xlsx"
```
...
- Confirm `src/excel-migration/logs/latest_import_metadata.json` was updated so
  runtime baseline drift checks can compare old diagrams against the current
  imported workbook.
- Compare the imported runtime against the workbook directly:

```bash
docker compose run --rm python-runner python /app/compare_excel_runtime.py "/app/excel-sheets/<excel-file>.xlsx"
```

- Expected happy path: `Overall: in-sync`
- If you see `Excel-only`, the workbook has rows that never reached runtime.
- If you see `Runtime-only`, old runtime rows are still hanging around after an
  earlier import.
- If you see `Value-mismatch`, treat it as manual review before trusting the
