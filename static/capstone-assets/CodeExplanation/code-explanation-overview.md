...
These pages form a maintenance map for the main application flows: dashboard and canvas entry, header controls, node and edge editing, diagram save and node cache persistence, subnetwork persistence, TP and Economic data, equation writing, Plant Measurements, run configuration, backend data routes, compute dispatch, solver callbacks, translation, reverse translation, and Excel import setup.
...
7. Read [Time Period and Economic Flow](./time-period-and-economic-flow.md), [Run Config and Computation Start](./run-config-and-computation-start.md), [Equation Writing Module Code Explanation](./equation-writing-module.md), and [Plant Measurements and Instrument Mapping Code Explanation](./plant-measurements-and-instrument-mapping.md) before changing run setup, TP data, Economic data, equations, Plant Measurements, or compute-start inputs.
...
| TP, Economic, equations, Plant Measurements, and run setup | [Time Period and Economic Flow](./time-period-and-economic-flow.md), [Run Config and Computation Start](./run-config-and-computation-start.md), [Equation Writing Module Code Explanation](./equation-writing-module.md), [Plant Measurements and Instrument Mapping Code Explanation](./plant-measurements-and-instrument-mapping.md) | Base TP, Multi-TP data, Economic cost ownership, equation persistence, instrument mappings, plant measurement import, run config selection, compute-start payload inputs, and generated parameter handoff. |
...
- Frontend interaction boundary: start from [Dashboard and Canvas Code Explanation](./dashboard-and-canvas.md) and [Header Bar Code Explanation](./header-bar.md), then follow the specific node, edge, modal, TP, Economic, equation, Plant Measurements, or save page.
