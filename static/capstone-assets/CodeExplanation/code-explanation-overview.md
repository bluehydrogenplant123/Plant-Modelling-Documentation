...
These pages form a maintenance map for the main application flows: dashboard and canvas entry, header controls, node and edge editing, diagram save and node cache persistence, subnetwork persistence, TP and Economic data, run configuration, Metabase/data visualization, backend data routes, compute dispatch, solver callbacks, translation, reverse translation, and Excel import setup.
...
- It documents the visualization boundary that turns PostgreSQL computation results into Metabase dashboards and generated chart panels.
...
7. Read [Time Period and Economic Flow](./time-period-and-economic-flow.md), [Run Config and Computation Start](./run-config-and-computation-start.md), and [Metabase and Data Visualization](./metabase-and-data-visualization.md) before changing run setup, TP data, Economic data, compute-start handoff, or TP Analysis visualization.
...
| TP, Economic, run setup, and visualization | [Time Period and Economic Flow](./time-period-and-economic-flow.md), [Run Config and Computation Start](./run-config-and-computation-start.md), [Metabase and Data Visualization](./metabase-and-data-visualization.md) | Base TP, Multi-TP data, Base-to-MTP value copy, Base/Multi-TP Specs, Economic cost ownership, run config selection, compute-start payload inputs, Metabase embedding, generated visualization panels, and dashboard binding metadata. |
...
- Visualization boundary: start from [Metabase and Data Visualization](./metabase-and-data-visualization.md), then follow [Computation Result Persistence](./computation-result.md) if the displayed data itself is missing or stale.
