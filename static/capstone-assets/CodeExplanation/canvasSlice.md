...
```
'idle'       鈫?No computation running
   鈫?
'waiting'    鈫?Queued, waiting for computation server
   鈫?
'computing'  鈫?Currently computing
   鈫?
'success'    鉁?Computation completed successfully
'failed'     鉁?Computation failed with error
'aborted'    鉁?User killed the computation
'timeout'    鉁?Exceeded max computation time
```
...
// What gets updated:
// 1. nodeParameters keys: 'BASE_TP-temp-node-1-*' 鈫?'BASE_TP-550e8400-*'
// 2. nodeRotations: nodeRotations['temp-node-1'] 鈫?nodeRotations['550e8400-*']
// 3. nodes list: node.node_id = 'temp-node-1' 鈫?'tail550e8400-*'
// 4. portMappings: portMappings['temp-node-1'] 鈫?portMappings['550e8400-*']
```
...
```typescript
// After saving, backend returns old 鈫?new ID mappings
...
### 8.1 nodeParameters 鈫?nodeCache
...
### 8.2 parentConnections 鈫?Instance Diagrams

- **parentConnections**: Stored in canvas slice for instance diagrams
- **Links**: Wrapper node in parent 鈫?internal nodes in instance
