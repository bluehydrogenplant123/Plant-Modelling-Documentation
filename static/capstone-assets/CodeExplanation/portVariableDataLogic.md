...
## 6. TP Changes (Spec/Bounds/Unit 瑕嗙洊)

**鐩殑锛?* 璁╅潪 Base TP 鐨勫彉鏇翠互绋€鐤忚鐩栫殑褰㈠紡鎸佷箙鍖栧埌 `tp_changes`锛屽苟鍦?UI 涓庢眰瑙ｄ晶姝ｇ‘鐢熸晥銆?
...
**鍏抽敭鐐癸細**
1. `NodeVarInput` 鍦ㄩ潪 Base TP 鐨?spec/units 鍙樺寲鏃讹紝浼氭妸浜嬩欢浜ょ粰 `NodeVarsTab` 缁熶竴淇濆瓨銆?
2. `NodeVarsTab` 浼氬皢 `portVarValue/spec/lowerBound/upperBound/unit` 涓庡熀绾块€愰」瀵规瘮锛屼换鎰忓瓧娈典笉鍚屽嵆 Upsert锛屽惁鍒?Delete銆?
3. Upsert 鏃朵細鍥炲～蹇呰鐨?`spec/unit`锛堝嵆浣挎湭鏀癸級锛岄伩鍏嶉娆′繚瀛樹负 `null`銆?
4. Backend 鍦?`translation` 闃舵灏?`tp_changes` 娉ㄥ叆 `ModelVersion`锛屾渶缁堣繘鍏?`tps_specs`銆?

璇︾粏娴佺▼瑙侊細`docs/CodeExplanation/tpChanges.md`
...
2. `NodeVarsTab` diffs `portVarValue/spec/lowerBound/upperBound/unit` against baseline. Any diff 鈫?Upsert; no diff 鈫?Delete.
