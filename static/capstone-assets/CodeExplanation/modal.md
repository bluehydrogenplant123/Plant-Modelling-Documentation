...
- `model: Model` 鈥?required; current model bound to the node.
- `onModelFieldChange(field: keyof Model, value: Model[K]) => void` 鈥?required; no return, pushes top-level model edits upward.
- `onNodeVarFieldChange(portsVarId: string, timePeriodId: string | undefined, field: keyof PortVarObject, value: PortVarObject[K]) => void` 鈥?required; no return, propagates variable edits with time-period context.
- `onSelectTimePeriod(timePeriodId?: string) => void` 鈥?optional id; no return, parent can load different versions.
- `nodeId: string` 鈥?required; React Flow node id for cache lookups.

## Core Functions
- `handleTabChange(tab)` 鈥?param: tab union; return: void; updates `activeTab` to show the right tab component.
- `updateTimePeriodPortVar({ model, timePeriodId, portVarId, newValue })` 鈥?params: `Model`, target time period id, variable id, numeric value; return: void; mutates the matching port var inside the selected time period in memory (currently uses `BASE_TP` placeholder).
- `handleVarChange(portsVarId, key, event)` 鈥?params: variable id, model var key, input/select event; return: void; normalizes number/string, updates cached modelVersion when editing `base_unit_default_value`, calls `onNodeVarFieldChange`, and syncs node cache for downstream consumers.
- `handlePortChange(event)` 鈥?param: select change; return: void; sets `selectedPort` for Node Vars tab (defaults from Redux domain model).
- `nodeCache.getCachedModelVersion(nodeId)` 鈥?used to seed and update cached versions; edits are persisted via `nodeCache.updateNodeValue`.
...
- `settings-tab.tsx`
  ...
  - Core: loads/syncs modelVersion via `useNodeCache`; validates unique node names (Redux `nodeNames` + `modelNameMap`); allows base model version switch with `initModelVersionWithCalcType` and refreshes node parameters via `refreshNodeParametersAfterModelVersionChange`; supports 鈥淕o to Subnetwork鈥?save-and-navigate using `useSaveDiagram`; shows model version metadata; blocks edits when verified.
...
- `specs-tab.tsx`
  ...
  - Returns JSX; mutations handled via `handleChange` 鈫?parent.
- `node-var-input.tsx`
