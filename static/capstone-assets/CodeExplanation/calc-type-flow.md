...
## Creation Flow
1. The 鈥淐reate Diagram鈥?modal presents the calc type dropdown using `CALC_TYPES`.
...
1. Header 鈥淐alc Type鈥?section renders four buttons: Simulation / Optimization / DataRec / ParamUpdt.
2. Clicking the current calc type opens the TP Specs panel immediately.
3. Clicking a different calc type shows a confirmation modal (鈥淪witch from X to Y?鈥?.
4. Confirm:
   - Updates Redux `calcType`.
   - Opens TP Specs panel (same component as Model 鈫?Specs).
5. TP Specs 鈥淪ave鈥?triggers:
...
- When `calcType === 'Simulation'`:
  - TP Specs grid 鈥淪pec鈥?column is read-only.
